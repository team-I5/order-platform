...
public class ProductController {
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "공개/숨김 상태 변경 성공",
                    content = @Content(schema = @Schema(implementation = ProductResponseDto.class)))
    })
    @PreAuthorize("hasRole('OWNER')")
    ...
    public ResponseEntity<ApiResponse<Void>> addGroupToProduct(@Valid @RequestBody ProductAddOptionGroupsRequestDto productAddOptionGroupsRequestDto) {
    ...
    public ResponseEntity<ApiResponse<ProductDetailResponseDto>> getProductList(
            @Parameter(description = "상품 ID") @PathVariable UUID productId
    ) {
