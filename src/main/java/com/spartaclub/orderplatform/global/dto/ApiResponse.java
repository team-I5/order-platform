package com.spartaclub.orderplatform.global.dto;
...
public class ApiResponse<T> { // 제네릭 타입 T를 사용하여 다양한 데이터 타입을 담을 수 있는 응답 클래스
    ...
    /**
     * 실패 응답 생성 메서드
     * 에러 메시지와 함께 실패 응답을 반환할 때 사용
     */
    public static <T> ApiResponse<T> error(String message) { // 에러 메시지를 받아 실패 응답 생성하는 정적 메서드
        return new ApiResponse<>(false, message, null); // success=false, 전달받은 에러 메시지, data=null로 객체 생성
    }
}