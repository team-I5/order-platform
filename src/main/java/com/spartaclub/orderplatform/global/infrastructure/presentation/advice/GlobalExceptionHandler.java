...
public class GlobalExceptionHandler { // 전역 예외 처리를 담당하는 클래스
    ...
    public ResponseEntity<ApiResponse<Void>> handleRuntimeException(
        ...
        // 로그인 실패 메시지인 경우 401 Unauthorized로 처리
        if (e.getMessage() != null && e.getMessage().contains("이메일 또는 비밀번호가 일치하지 않습니다")) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED) // HTTP 401 상태코드로 응답 설정
                .body(ApiResponse.error(e.getMessage())); // 에러 메시지와 함께 실패 응답 반환
        }
